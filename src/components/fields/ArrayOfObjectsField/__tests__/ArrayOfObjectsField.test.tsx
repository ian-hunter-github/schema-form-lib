...
describe("ArrayOfObjectsField", () => {
  ...
  it("renders empty array with placeholder message", () => {
    ...
    expect(screen.getByTestId("testArray-label")).toHaveTextContent(
      "TestArray"
    );
  ...
  it("expands and collapses items when header is clicked", () => {
    ...
    expect(
      screen.queryByTestId("mock-field-testArray.0.name")
    ...
    expect(
      screen.getByTestId("mock-field-testArray.0.name")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("mock-field-testArray.0.age")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("mock-field-testArray.0.active")
    ).toBeInTheDocument();
  ...
  it("handles nested field changes correctly", async () => {
    ...
    expect(mockFormModel.setValue).toHaveBeenCalledWith(
      "testArray.0.name",
      "Jane"
    );
  });
